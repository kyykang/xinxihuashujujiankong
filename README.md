...
- **数据库监控**：MySQL/SQL Server 数据库连接状态
- **存储监控**：磁盘空间使用情况
- **业务指标监控**：自定义 SQL 查询监控业务数据
- **备份文件监控**：远程服务器备份文件检查和告警
...
### 🔒 安全特性
- **用户认证**：登录/注销功能，会话管理
- **权限控制**：管理员和普通用户角色，基于角色的访问控制
- **密码加密存储**：使用 AES 加密算法保护敏感信息
- **自动加密**：所有密码自动加密后存储到数据库
- **密钥管理**：安全的密钥文件管理机制
- **向后兼容**：支持现有数据的加密迁移
...
**重要**：首次启动后，请立即备份 `.secret_key` 文件到安全位置！
...
# 添加管理员
python3 add_user.py admin2 admin456 "admin2@example.com" 1
...
- 用户管理：http://localhost:8080/users（仅管理员）
...
详细说明请参考：[用户权限管理说明.md](用户权限管理说明.md)
...
# 将备份保存到安全位置
# 如：加密的U盘、密码管理器、安全的云存储等
```

#### 密钥丢失

如果密钥文件丢失：
1. 无法恢复已加密的密码（加密是不可逆的）
2. 需要删除数据库，重新配置所有监控目标
3. 预防措施：定期备份密钥文件
...
├── migrate_encrypt_passwords.py # 密码加密迁移脚本
├── requirements.txt            # Python 依赖
├── .secret_key                 # 加密密钥（自动生成，不提交到Git）
├── templates/                  # HTML 模板
│   ├── base.html
│   ├── dashboard.html
│   ├── targets.html
